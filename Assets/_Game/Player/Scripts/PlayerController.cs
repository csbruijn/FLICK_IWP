...
public class PlayerController : MonoBehaviour, IPlayerController
{
    ...
    private void LateUpdate()
    {
        ...
        previousTimeInAirBoost = TimeInAirBoost;
    }
    ...
    {
        if (HasAddedBoost) { Debug.LogWarning("multiple boosts trying to apply"); return; }
        
        HasAddedBoost = true;

        TimeInAirBoost += Time.deltaTime;
            
        float boostStr = strength *= TimeInAirBoost;  

        _frameVelocity.y = Mathf.Max(_frameVelocity.y, boostStr);
    }
